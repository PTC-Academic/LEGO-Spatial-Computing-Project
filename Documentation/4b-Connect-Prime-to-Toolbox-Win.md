...
9. Open a new Command Prompt window by clicking the Windows button on the keyboard and enter ```Command```. This is where the commands to start the Vuforia Spatial Edge Server will be executed from.
...
11. Type ```node -v``` to check which version of Node.js was installed and take note of the result.
12. Run ```npm install``` while in the _SpatialToolbox-Windows-Interns/vuforia-spatial-edge-server_ directory
